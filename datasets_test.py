...
class PairedImgDataset(GeneratorDataset):
    def __init__(self, data_source, mode, crop=[256, 256], random_resize=None):
        ...
        if self.mode == 'train':
            self.img_paths_l = sorted(glob.glob(data_source + '/train/low/' + '*L.*'))
            self.img_paths_r = sorted(glob.glob(data_source + '/train/low/' + '*R.*'))
            self.gt_paths_l = sorted(glob.glob(data_source + '/train/gt/' + '*L.*'))
            self.gt_paths_r = sorted(glob.glob(data_source + '/train/gt/' + '*R.*'))
        else:
            self.img_paths_l = sorted(glob.glob(data_source + '/low/' + '*L.*'))
            self.img_paths_r = sorted(glob.glob(data_source + '/low/' + '*R.*'))
            self.gt_paths_l = sorted(glob.glob(data_source + '/gt/' + '*L.*'))
            self.gt_paths_r = sorted(glob.glob(data_source + '/gt/' + '*R.*'))
...
class SingleImgDataset(GeneratorDataset):
